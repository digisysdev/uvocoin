...
#if defined(NDEBUG)
# error "Uvocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf760fc1401134372a439ac093c575d99d54160b3b0110c8a106066aa263bca18");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Uvocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Uvocoin Signed Message:\n";

double dHashesPerSec = 0.0;
int64 nHPSTimerStart = 0;

// Settings
int64 nTransactionFee = 0.00001;
...
{
    // Uvocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5000000); // Uvocoin: 5000k blocks in ~4 years
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Uvocoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Uvocoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Uvocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Uvocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf760fc1401134372a439ac093c575d99d54160b3b0110c8a106066aa263bca18");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is uvocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xa6d706ea90bed2116c76eed53074f26e0d24d8311e864ed2ce567736307ecccf"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Uvocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UvocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UvocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UvocoinMiner(CWallet *pwallet)
{
    printf("UvocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("uvocoin-miner");
    ...
    try { loop {
        ...
        printf("Running UvocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UvocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UvocoinMiner, pwallet));
}
